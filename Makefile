...
pyinstaller:
	mkdir -p dist
	pyinstaller -y --workpath pyinstaller/build --distpath pyinstaller/dist rhasspyasr_kaldi_hermes.spec
	tar -C pyinstaller/dist -czf dist/rhasspy-asr-kaldi-hermes_$(version)_$(architecture).tar.gz rhasspyasr_kaldi_hermes/
...
debian: pyinstaller
	...
	cp -R pyinstaller/dist/rhasspyasr_kaldi_hermes "$(debian_dir)/usr/lib/"
	cd debian/ && fakeroot dpkg --build "$(debian_package)"
	mv "debian/$(debian_package).deb" dist/

docker: pyinstaller
	docker build . -t "rhasspy/rhasspy-asr-kaldi-hermes:$(version)"
